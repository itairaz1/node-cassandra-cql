...
var encoder = (function(){
  ...
  function encode (item) {
    ...
    if (item.hint) {
      ...
      if (typeof type === 'string') {
        ...
        subtype = typeInfo.subtype;
          keysubtype = typeInfo.keysubtype;
          valuesubtype = typeInfo.valuesubtype;
      }
    }
    ...
    switch (type) {
      ...
      case dataTypes.map:
        return encodeMap(value, type, keysubtype, valuesubtype);
      default:
  ...
  function encodeInt (value) {
    if (typeof value !== 'number') {
      throw new TypeError(value +' is not a number', value, 'number');
    }
  ...
  function encodeBigNumber (value, type) {
    ...
    if (buf === null) {
      throw new TypeError(value +' is not a buffer', value, Buffer, null, type);
    }
    return buf;
  }
  ...
  function encodeString (value, encoding) {
    if (typeof value !== 'string') {
      throw new TypeError(value +' is not a string', value, 'string');
    }
  ...
  function encodeBlob (value, type) {
    if (!(value instanceof Buffer)) {
      throw new TypeError(value +' is not a blob', value, Buffer, null, type);
    }
    return value;
  }
  ...
  function encodeList(value, type, subtype) {
    if (!util.isArray(value)) {
      throw new TypeError(value +' is not an array', value, Array, null, type);
    }
    if (value.length === 0) {
      return null;
    }
  ...
  function encodeMap(value, type, keysubtype, valuesubtype) {
    var parts = [];
    var propCounter = 0;
    for (var key in value) {
      if (!value.hasOwnProperty(key)) continue;
      //add the key and the value
        var keyBuffer = encode(keysubtype ? {value:key, hint: keysubtype}: key);
        //include item byte length
      parts.push(getLengthBuffer(keyBuffer));
      //include item
      parts.push(keyBuffer);
      //value
        var valueBuffer = encode(valuesubtype ? {value:value[key], hint: valuesubtype}: value[key]);
        //include item byte length
